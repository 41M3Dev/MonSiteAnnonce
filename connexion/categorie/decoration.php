...
$check = $bdd->prepare( "SELECT `titre`, `description`, `localisation`, `contact`, `categorie` FROM `annonce` WHERE `categorie` LIKE 'Decoration'"  );
            ...
            $comp = $check->rowCount();
                if( $comp == 0 ){
                    ...
                    <div>
                        <?php 
                            echo "Na pas d'annonce gars RIP pour toi ";
                        ?>
        ...
        // Affihce les valeur de la base de donnée
            $sql="SELECT `titre`, `description`, `localisation`, `contact`, `categorie` , `pseudo` FROM `annonce` WHERE `categorie` LIKE 'Decoration'";
            ...
            <section>
                <div>
                    <?php echo "<h1>{$row['titre']}</h1>
                         ...
                         echo ' <a href="mailto:'.$row['contact'].'"><h3>Mail</h3></a> '; 
                        // echo ' <a href="../message/message.php?pseudo='.$row['pseudo'].'"><h3>Message privé</h3></a> '; ?>
